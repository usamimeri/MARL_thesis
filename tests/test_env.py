...
def test_compute_firm_labor_simple(env):
    ...
    env.worker_in_firm = np.array([0, 1, 1, 2, 0])
    env.worker_labor = np.array([100.0, 200.0, 300.0, 400.0, 100.0])
    expected = np.array([200.0, 500.0, 400.0])
...
def test_compute_firm_labor_zero(env):
    ...
    env.worker_labor = np.array([100, 200, 300, 400, 100])
    env.worker_in_firm = np.array([0, 1, 1, 2, 0])
    expected = np.array([200., 500., 400.])
    result = env.compute_firm_labor()
    assert np.allclose(result, expected), f"Expected {expected}, got {result}"
...
def test_market_clearing(env):
    env.reset()
    env.worker_consumption = np.array([20, 10, 5], dtype=np.float32)  # num_worker
    env.worker_quote = np.array([100, 90, 80], dtype=np.float32)  # num_worker
    env.firm_production = np.array([10, 15, 10], dtype=np.float32)  # num_firm
    env.firm_quote = np.array([70, 75, 120], dtype=np.float32)  # num_firm
    env.market_clearing()
...
def test_government_settlement(env, config):
    ...
    worker_obs = torch.tensor(env.construct_worker_obs(), dtype=torch.float32).to(env.device)
    worker_action = worker_net.get_logprob_and_action(worker_obs)[2].cpu().numpy()
    firm_obs = torch.tensor(env.construct_firm_obs(), dtype=torch.float32).to(env.device)
    firm_action = firm_net.get_logprob_and_action(firm_obs)[2].cpu().numpy()
    government_obs = torch.tensor(env.construct_government_obs(), dtype=torch.float32).to(env.device)
    government_action = government_net.get_logprob_and_action(government_obs)[2].cpu().numpy()
